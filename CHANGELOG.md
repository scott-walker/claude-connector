...
- **A cancel that arrived before the turn started was ignored.** `interrupt()` only stops work the worker has already picked up, so aborting in the gap between `init()` and the turn beginning did nothing and the turn ran to completion — billed and discarded. Measured against the real CLI: an abort 900 ms into a cold client used to run the full 17.5 s turn and report `subtype: 'success'`; it now ends in 2.9 s with `error_during_execution`. The cancel is retried on a short cadence until the turn stops, and `stream()` no longer yields the cancelled turn's remaining text — only its result, so the consumer sees the turn close.
- **A truncating resume repeated on every turn.** `Session` forwarded `resumeSessionAt` / `resumeDropsTurn` on each query, so from the second turn on the transcript was re-truncated back to the same chain entry. Both are now first-turn-only, which is what a truncating resume means.
- **A per-query permissions override wiped the client's own settings.** The flag-settings mirror was seeded only from an inline `settings` *object*, but the facade forwards settings to the SDK as serialized JSON — so through `new Claude(...)` the mirror was always empty, and restoring after a per-query `allowedTools` / `disallowedTools` / `additionalDirs` override cleared the flag layer for the rest of the session.
- **A failed CLI spawn looked like a clean end of stream.** The stream iterator tested its `done` flag before its `error` field, and the child's `error` handler sets `done` — so a spawn failure resolved the iterator as a finished stream instead of rejecting.
- **An agent without a prompt failed from inside the SDK.** `agents` entries are validated at construction now, so the error names the agent instead of surfacing from the SDK's arg builder.
